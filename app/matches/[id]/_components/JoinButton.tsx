...
export default function JoinButton({
  ...
  useEffect(() => {
    if (joinState.success) router.push(`/matches/${matchId}/pay`);
    ...
    <div className="space-y-3">
      {leaveState.message && (
        <div className={["text-sm rounded-xl px-4 py-3 border",
          leaveState.success ? "bg-green-950 border-green-800 text-green-400" : "bg-red-950 border-red-800 text-red-400"].join(" ")}>
          {leaveState.message}
        </div>
      )}
            ...
            {canLeave && (
              <form action={leaveAction}>
                ...
                <button type="submit" disabled={leavePending}
                  className="shrink-0 text-xs text-zinc-500 border border-white/[0.08] px-3 py-1.5 rounded-lg
                             hover:text-red-400 hover:border-red-800 transition-colors disabled:opacity-50">
      ...
      {!canJoin && !isRegistered && matchStatus === "FULL" && (
        <div className="w-full bg-[#0A0A0A] border border-white/[0.08] text-zinc-500 rounded-xl py-3 text-sm text-center">
          Все места заняты
        </div>
      )}
    </div>
