...
interface DV {
  firstName: string; lastName: string; age: number; heightCm: number; weightKg: number;
  mainPosition: string; altPosition: string; dominantFoot: string;
  experienceLevel: string; experienceYears: number; phone: string;
}
...
export default function SetupForm({ defaultValues: d }: { defaultValues: DV }) {
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      <nav className="border-b border-white/[0.08] px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
        ...
        <Link href="/dashboard" className="text-zinc-500 text-sm hover:text-white transition-colors">← Назад</Link>
      </nav>

      <div className="max-w-lg mx-auto px-4 sm:px-6 py-8 sm:py-10 page-fade">
        <h1 className="font-display text-4xl tracking-wide mb-1">Заполните профиль</h1>
        <p className="text-zinc-500 text-sm mb-8">Эти данные помогут подобрать подходящие матчи</p>

        {state.message && !state.fieldErrors && (
          <div className={["text-sm rounded-xl px-4 py-3 border mb-6",
        ...
        <form action={formAction} className="space-y-5">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input name="firstName" type="text" defaultValue={d.firstName} placeholder="Азиз" required className={cls(state.fieldErrors?.firstName?.[0])} />
            ...
            <div>
              ...
              <input name="lastName" type="text" defaultValue={d.lastName} placeholder="Турсунов" required className={cls(state.fieldErrors?.lastName?.[0])} />
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input name="age" type="number" defaultValue={d.age || ""} placeholder="25" min="14" max="60" required className={cls(state.fieldErrors?.age?.[0])} />
            </div>
            <div>
              <label className="block text-zinc-400 text-sm mb-1.5">Рост (см)</label>
              <input name="heightCm" type="number" defaultValue={d.heightCm || ""} placeholder="178" min="140" max="220" required className={cls(state.fieldErrors?.heightCm?.[0])} />
            </div>
            <div>
              <label className="block text-zinc-400 text-sm mb-1.5">Вес (кг)</label>
              <input name="weightKg" type="number" defaultValue={d.weightKg || ""} placeholder="75" min="40" max="150" required className={cls(state.fieldErrors?.weightKg?.[0])} />
            </div>
          </div>
          ...
          <div>
            <label className="block text-zinc-400 text-sm mb-1.5">Дополнительная позиция <span className="text-zinc-600">(необязательно)</span></label>
          ...
          <div>
            ...
            <input name="experienceYears" type="number" defaultValue={d.experienceYears} placeholder="3" min="0" max="50" className={cls()} />
          </div>

          <div>
            <label className="block text-zinc-400 text-sm mb-1.5">Телефон <span className="text-zinc-600">(необязательно)</span></label>
            <input name="phone" type="tel" defaultValue={d.phone} placeholder="+998 90 123 45 67" className={cls(state.fieldErrors?.phone?.[0])} />
          </div>
